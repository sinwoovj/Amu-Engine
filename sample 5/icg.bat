...
:CREATE_PROJECT_TUTORIAL_1
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/tutorial-1.zip projects
	GOTO BUILDCMAKELISTS

:CREATE_PROJECT_TUTORIAL_2
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/tutorial-2.zip projects
	GOTO BUILDCMAKELISTS

:CREATE_PROJECT_TUTORIAL_3
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/tutorial-3.zip projects
	GOTO BUILDCMAKELISTS

:CREATE_PROJECT_TUTORIAL_4
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/tutorial-4.zip projects
...
:CREATE_PROJECT_TUTORIAL_5
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/tutorial-5.zip projects
...
:COPY_RESOURCE
	...
	if not exist "%source-path%" (
	    echo [WARNING!] %prj1% directory does not exists.
	    pause
	    GOTO MENU
	) else (
...
:BUILDCMAKELISTS
	...
	(
		...
		ECHO.
		ECHO add_definitions(-DUSE_CSD2101_AUTOMATION=0^)
	    ...
	    ECHO     ${CMAKE_CURRENT_LIST_DIR}/lib/glfw-event-recorder/hook_csd2101.h
		ECHO ^)
		ECHO.
		...
		ECHO ADD_LIBRARY(csd2101 STATIC
	    ECHO     ${EXTERNAL_LIBRARY_SOURCES}
	    ECHO ^)
		ECHO target_link_libraries(csd2101
		ECHO     ${ALL_LIBS}
		ECHO ^) 
